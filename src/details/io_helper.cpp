...
int IOHelper::CreateSocket(bool is_non_block) {
  ...
  XFC_FAIL_HANDLE(fd<=0 && (ret=-1))

  if (is_non_block) {
    ret = SetNonBlock(fd);
    XFC_FAIL_HANDLE_AND_SET(true!=ret, ret=-2)
  }

  on=1;
  ret = setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on));
  XFC_FAIL_HANDLE_AND_SET(0!=ret, ret=-3)
  return fd;
...
int IOHelper::Listen(const sockaddr_in& addr, size_t backlog) {
  int ret;
  int fd = CreateSocket();
  XFC_FAIL_HANDLE_AND_SET(fd<=0, ret=-1)

  ret = bind(fd, RCAST<const sockaddr*>(&addr), sizeof(addr)); 
  XFC_FAIL_HANDLE_AND_SET(0!=ret, ret=-2)
