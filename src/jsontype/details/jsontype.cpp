...
void SharedJsonVal::DecRefCnt(bool use_pool) {
  ...
  if (0==ref_cnt) {
    if (use_pool) {
      ...
      switch (type) {
        case JsonValType::kStr : XFC_DELETE(data.str_val); break;
        case JsonValType::kList : XFC_DELETE(data.list_val); break;
        case JsonValType::kDict : XFC_DELETE(data.dict_val); break;
        default : break;
      }
    }
  }
...
SharedJsonVal* SharedJsonVal::Copy() {
  XFC_NEW_DECL(shared_json_val, SharedJsonVal, SharedJsonVal(type))
  switch (type) {
...
JsonType* SharedJsonVal::JsonParseUtil::ParseJson(const char*& str) {
  ...
  switch (*str) {
    ...
    case 'n' : {
      json = ReadinNull_(str);
      XFC_FAIL_HANDLE(NULL==json)
      return json;
    }
    case 't' : {
      json = ReadinTrue_(str);
      XFC_FAIL_HANDLE(NULL==json)
      return json;
    }
    case 'f' : {
      json = ReadinFalse_(str);
      XFC_FAIL_HANDLE(NULL==json)
      return json;
    }
    case '"' : {
    ...
    case '[' : {
      json = ReadinList_(str); 
      XFC_FAIL_HANDLE(NULL==json)
      return json;
    }
    case '{' : {
      json = ReadinDict_(str);
      XFC_FAIL_HANDLE(NULL==json)
      return json;
    }
    default : 
      if ((*str>='0' && *str<='9') || *str == '-') {
        json = ReadInDigits_(str);
        XFC_FAIL_HANDLE(NULL==json)
        return json;
      } else {
        XFC_FAIL_HANDLE(true)
...
JsonType* SharedJsonVal::JsonParseUtil::ReadinNull_(const char*& str) {
  ...
  str+=len_null;
  XFC_NEW_DECL(null_val, JsonType, JsonType(JsonValType::kNull))
  return null_val;
}
...
JsonType* SharedJsonVal::JsonParseUtil::ReadinTrue_(const char*& str) {
  ...
  str+=len_true;
  XFC_NEW_DECL(bool_val, JsonType, JsonType(JsonValType::kBool))
...
JsonType* SharedJsonVal::JsonParseUtil::ReadinStr_(const char*& str) {
  XFC_NEW_DECL(str_val, JsonType, JsonType(JsonValType::kStr))
...
JsonType* SharedJsonVal::JsonParseUtil::ReadinList_(const char*& str) {
  ...
  XFC_NEW_DECL(list_val, JsonType, JsonType(JsonValType::kList))
  while (']'!=*str && '\0'!=*str) {
    IgnoreBlanks_(str);
...
JsonType* SharedJsonVal::JsonParseUtil::ReadinDict_(const char*& str) {
  ...
  XFC_NEW_DECL(dict_val, JsonType, JsonType(JsonValType::kDict))
  while ('}'!=*str && '\0'!=*str) {
    IgnoreBlanks_(str);
...
JsonType* SharedJsonVal::JsonParseUtil::ReadInDigits_(const char*& str) {
  ...
  if ('.' == *endptr) {
    ...
    XFC_NEW_DECL(int_val, JsonType, JsonType(JsonValType::kDouble))
    int_val->shared_json_val_->data.double_val = d;
    str=endptr;
    return int_val;
  } else {
    XFC_NEW_DECL(int_val, JsonType, JsonType(JsonValType::kInt))
...
std::string* SharedJsonVal::JsonParseUtil::ReadinStrRaw_(const char*& str) {
  ...
  if ('\0'==*str) {
    ...
    XFC_NEW_DECL(str_val, std::string, std::string)
...
SharedJsonVal::JsonParseUtil::ReadDictPair_(const char*& str) {
  ...
  XFC_FAIL_HANDLE(NULL==str_val || 0 == str_val->length())
...
#define ASSIGN_NAIVE_TYPE(data_type, val_type, data_member) \
  JsonType& JsonType::operator=(data_type data_val) { \
    ...
    \
    if (NULL==shared_json_val_) { \
      XFC_NEW(shared_json_val_, SharedJsonVal(val_type)) \
    } else { \
...
JsonType& JsonType::operator=(const char* str_val) {
  ...
  if (NULL==shared_json_val_) {
    XFC_NEW(shared_json_val_, SharedJsonVal(JsonValType::kStr))
  } else {
...
const JsonType* JsonType::ParseJson(const char* str) {
...
const JsonType* JsonType::CreateConf(const std::string& filepath) {
